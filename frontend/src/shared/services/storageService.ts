...
export class StorageService {
  static saveCVData(cvData: CVData): boolean {
    try {
      ...
      if (error instanceof DOMException && error.name === 'QuotaExceededError') {
        console.error('⚠️ localStorage lleno. Considera limpiar versiones antiguas.');
      }
      return false;
    }
  }
  ...
  static importAllData(jsonData: string): boolean {
    try {
      ...
      if (data.cvData) this.saveCVData(data.cvData);
      if (data.history && Array.isArray(data.history)) {
        localStorage.setItem(STORAGE_KEYS.CV_HISTORY, JSON.stringify(data.history));
      }
  ...
  static getStorageInfo(): { used: string; usedBytes: number; available: string; percentUsed: number } {
    try {
      let totalSize = 0;
      ...
      return { used: `${usedKB} KB`, usedBytes: totalSize, available: `${availableKB} KB`, percentUsed: parseFloat(percentUsed) };
    } catch {
      return { used: 'N/A', usedBytes: 0, available: 'N/A', percentUsed: 0 };
    }
  }

  static getItemCount(): { cvVersions: number; coverLetters: number; hasCurrentCV: boolean } {
    return {
