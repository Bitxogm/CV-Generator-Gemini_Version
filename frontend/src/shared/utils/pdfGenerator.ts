...
const generateFormalPDF = (
  ...
  if (options?.candidateName) {
    doc.text(options.candidateName, pageWidth - marginRight, yPosition, {
      align: "right",
    });
    yPosition += 4;
  }
  ...
  if (options?.email) {
    doc.text(options.email, pageWidth - marginRight, yPosition, {
      align: "right",
    });
    yPosition += 4;
  }
  ...
  if (options?.phone) {
    doc.text(options.phone, pageWidth - marginRight, yPosition, {
      align: "right",
    });
    yPosition += 4;
  }
  ...
  if (options?.location) {
    doc.text(options.location, pageWidth - marginRight, yPosition, {
      align: "right",
    });
    yPosition += 4;
  }
  ...
  if (options?.linkedin) {
    const linkedinText = options.linkedin
      .replace("https://", "")
      .replace("http://", "");
    doc.text(linkedinText, pageWidth - marginRight, yPosition, {
      align: "right",
    });
    yPosition += 4;
  }
...
const renderContentInOnePage = (
  ...
  for (let i = 0; i < fontSizes.length; i++) {
    ...
    const testHeight = calculateTextHeight(
      doc,
      text,
      contentWidth,
      fontSize,
...
const generateFileName = (options?: PDFOptions): string => {
  ...
  const prefix =
    options?.language === "en" ? "cover_letter" : "carta_presentacion";
